...
**D4. 后端与前端形态**
- ~~后端：`reviews/backend/` FastAPI（端口 8503）~~ → **v1 修订为纯前端演示层**：`reviews/frontend/index.html` 内置 JS 版观点提取与情感词典（浏览器端完成 评价→观点 分析与全部可视化），`libs/` 本地化依赖，**双击即可运行**，与 `dashboard/` 的「本地优先」定位一致。
- Python 后端保留为**可选增强**（v1.5+）：接入 `huggingface` / `llm` analyzer 时再启用 FastAPI（`reviews/backend/` 已具备 lexicon 分析、导入标准化与痛点挖掘核心，`ANALYZER` 环境变量约定不变）。
- 理由：分析结果以图表/表格为主；纯前端让面试演示与日常使用零安装、零服务器，且与既有运营看板体验统一。
- 备选：React（与 `frontend/` 统一）——v2 若交互复杂化再迁移。

**D6. 观点（aspect-opinion）数据模型（v1 前端落地，2026-08-07 修订）**
- 每条评价经规则版观点提取器拆出多条 `观点`：`{aspect 属性, group 一级标签, word 评估词, sentiment 正/负面, phrase}`。
- 前端按 属性×情感 聚合出：观点标签排名（负面/正面/整体切换）、属性情感对比（肤感/气味/使用感/价格…堆叠条形）、观点词频表、商品声量排名。
